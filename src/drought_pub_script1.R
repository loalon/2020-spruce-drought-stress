#' ---
#' title: "RNA-Seq analysis of spruce-drought-stress"
#' author: "Julia Hass, Alexander Vergara & Alonso Serrano"
#' date: "`r Sys.Date()`"
#' output:
#'  html_document:
#'    toc: true
...
resultsNeedles$control_vs_severe_drought <- resCL
...
resultsNeedles$control_vs_re_irrigation <- resCR
...
resultsRoots$control_vs_mild_drought <- resrootE
...
resultsRoots$control_vs_severe_drought <- resrootCL
...
resultsRoots$control_vs_re_irrigation <- resrootCR
...
#' ## DESEQ combined
#remove the low expressed genes
ddsFull <- DESeqDataSetFromMatrix(as.matrix(nr_filtered), samples_combined,formula(~Type))
ddsFull

ddsNR <- DESeq(ddsFull)

resultsCombined <- list(root_vs_needle = results(ddsNR,contrast=c("Type","Root","Needle"))) 
...
save(resultsNeedles, resultsRoots, resultsCombined, file="/mnt/picea/projects/spruce/vhurry/drought-stress-combined/de/deResults.RData")
...
vst<-rbind(Eneedleuponlyvst,ECLneedleuponlyvst,
           CLneedleuponlyvst,CLCRneedleuponlyvst,
           CRneedleuponlyvst,ECRneedleuponlyvst,
           commonneedleuponlyvst,needlerootuponlyvst,
           Erootuponlyvst,ECLrootuponlyvst,
           CLrootuponlyvst,CLCRrootuponlyvst,
           CRrootuponlyvst,ECRrootuponlyvst,
           commonrootuponlyvst)
annotation_row = data.frame( GeneClass = factor(rep(c("a", "b", "c","d","e","f","g","h","i","j","k","l","m","n","o"), 
                                                    c(nrow(Eneedleuponlyvst),nrow(ECLneedleuponlyvst),nrow(CLneedleuponlyvst),
                                                    1,#nrow(CLCRneedleuponlyvst),
                                                    nrow(CRneedleuponlyvst),
                                                    1,#nrow(ECRneedleuponlyvst),
                                                    nrow(commonneedleuponlyvst),nrow(needlerootuponlyvst),nrow(Erootuponlyvst),
                                                    nrow(ECLrootuponlyvst),nrow(CLrootuponlyvst),nrow(CLCRrootuponlyvst),
                                                    nrow(CRrootuponlyvst),nrow(ECRrootuponlyvst),nrow(commonrootuponlyvst))
                                                    #c(56,3,400,1,4,1,2,451,7,29,1236,192,278,2,17)
                                                    )))
head(annotation_row)
head(vst)
...
stageColors <- c("control"="#1B9E77", "M60"="#D95F02",
                "M40"="#7570B3","M30"="#E7298A",
                "M307d"="#66A61E","S"="#FFCC00",
                "S48h"="#996600","R"="#666666")

df = data.frame(stage = factor(rep(c("control","M60","M40","M30","M307d","S","S48h","R"), 1,Time=1:11)),
                tissue = factor(rep(c(rep("Needle", 8)))))

ha_a = HeatmapAnnotation(df=df, col=list(stage=stageColors,tissue =  c("Needle"="green", "Root"="brown")),
                         annotation_height = unit(3, "mm"), show_annotation_name=F)

ht1 = Heatmap(vst_1,show_row_names=F,cluster_columns=F, cluster_rows = F,
              show_row_dend=F,show_column_dend=F,
              name="vst",
              split = annotation_row$GeneClass,
              col = colorRamp2(c(-5, 0, 5), c("blue", "white", "red")),
              gap=unit(rep(2,14),"mm"),
              
              top_annotation=ha_a)

df = data.frame(stage = factor(rep(c("control","M60","M40","M30","M307d","S","S48h","R"), 1,Time=1:11)),
                tissue = factor(rep(c(rep("Root", 8)))))
ha_b = HeatmapAnnotation(df = df,
                         col = list(stage=stageColors,
                                    tissue =  c("Needle"="green", "Root"="brown")),annotation_height = unit(3, "mm"))
ht2 = Heatmap(vst_2,show_row_names=F,cluster_columns=F,cluster_rows = F,
              show_row_dend=F,show_column_dend=F,
              split = annotation_row$GeneClass,
              col = colorRamp2(c(-5, 0, 5), c("blue", "white", "red")),
              gap=unit(rep(2,14),"mm"),
              show_heatmap_legend = FALSE,top_annotation=ha_b)

colors<-c("#CCCC33","#FFFF00","#CCCC99" ,"#CCCCCC","#33FF00","#339900","#CCFF33","#FFFFCC","#FFCC00","#FF9900","#FFCC99","#996666","#CC6633","#CC3300","#CC9900")
hmMatrix <- as.matrix(data.frame(type=annotation_row$GeneClass))
ht_11<-Heatmap(hmMatrix, 
               col=colors, name="type",
               width=unit(5, "mm"))
...
vst_d<-rbind(Eneedledownonlyvst,ECLneedledownonlyvst,CLneedledownonlyvst,
             CRneedledownonlyvst,needlerootdownonlyvst,ECLrootdownonlyvst,
             CLrootdownonlyvst,CLCRrootdownonlyvst,CRrootdownonlyvst,
             commonrootdownonlyvst,N_R_opp,N_oppvst,
             N_oppvst)

#annotation_row = data.frame( GeneClass = factor(rep(c("a", "b", "c","d","e","f","g","h","i","j","k","l","m"), c(3,1,290,5,130,5,3531,183,58,2,55,1,18))))
annotation_row = data.frame( GeneClass = factor(rep(c("a", "b", "c","d","e","f","g","h","i","j","k","l","m"), 
                                                    c(nrow(Eneedledownonlyvst),
                                                      1, #nrow(ECLneedledownonlyvst),
                                                      nrow(CLneedledownonlyvst),
                                                      nrow(CRneedledownonlyvst),nrow(needlerootdownonlyvst),nrow(ECLrootdownonlyvst),
                                                      nrow(CLrootdownonlyvst),nrow(CLCRrootdownonlyvst),nrow(CRrootdownonlyvst),
                                                      nrow(commonrootdownonlyvst),nrow(N_R_opp),
                                                      1, #nrow(N_oppvst),
                                                      1 #nrow(N_oppvst)
                                                      )
...
summary(vst_d)
ht3<-Heatmap(vst_3,show_row_names=FALSE,cluster_columns=FALSE, cluster_rows=F,
             show_row_dend=FALSE,show_column_dend=FALSE,name="vst",
             split = annotation_row$GeneClass,
             col = colorRamp2(c(-5, 0, 5), c("blue", "white", "red")),
             gap=unit(rep(2,12),"mm"),
             top_annotation = ha_a)

ht4<-Heatmap(vst_4,show_row_names=FALSE,cluster_columns=FALSE,cluster_rows=F,
             show_row_dend=FALSE,show_column_dend=FALSE,
             split = annotation_row$GeneClass,
             col = colorRamp2(c(-5, 0, 5), c("blue", "white", "red")),
             gap=unit(rep(2,12),"mm"),
             show_heatmap_legend = F,top_annotation=ha_b)
...
colors<-c("#CCCC33","#FFFF00","#CCCC99","#33FF00","#FFFFCC",
          "#FF9900","#FFCC99","#996666","#CC6633","#CC9900",
          "#660066","#00FF99","#663300")
hmMatrix <- as.matrix(data.frame(type=annotation_row$GeneClass))
ht_33<-Heatmap(hmMatrix,col =colors,name="type",width = unit(5, "mm"))                             
